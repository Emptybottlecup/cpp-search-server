...
public:
    ...
    explicit SearchServer(const StringContainer& stop_words)
        ...
        for (auto stop_word : stop_words) {
            if (!(IsValidWord(stop_word))) {
                throw invalid_argument("Îäíî èç ïåðåäàííûõ ñòîï ñëîâ ñîäåðæèò íåäîïóñòèìûå ñèìâîëû ñ êîäàìè îò 0 äî 31");
            }
        }
    }
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status,
        ...
        if (documents_.count(document_id) != 0 || document_id < 0 || !IsValidWord(document)) {
            throw invalid_argument("Îøèáêà â äîáàâëåíèè äîêóìåíòà");
        }
    ...
    int GetDocumentId(int index) {
        if (index >= 0 && index <= documents_.size()) {
            ...
            throw out_of_range("Íå ñîîòâåòñâóþùèé Id");
        }
    }
